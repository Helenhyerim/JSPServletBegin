...
<body>
	<h2>문자열로 되어진 숫자를 누적한 결과값(자동형변환) 나타내기(JSTL을 사용)</h2>
	...
	<h2>문자열로 되어진 숫자를 정수로 형변환하여 누적한 결과값 나타내기(JSTL을 사용)</h2>
	<c:set var="sum" value="0" />
	<c:if test="${not empty requestScope.pointArr1}">
		<ul style="list-style-type: circle;">
			<c:forEach var="point" items="${requestScope.pointArr1}">
				<li>
					...
					<%--
						fmt:parseNumber 은 문자열을 숫자형식으로 형변환하는 것이다.
						integerOnly="true" 은 소수점은 절삭해버리고, 정수만 취하는 것이다.
	...
	<c:if test="${not empty requestScope.pointArr1}">
		<ul>
			<c:forEach var="point" items="${requestScope.pointArr1}" varStatus="anything">
				<li>${point}&nbsp;(인텍스번호 &nbsp; ${anything.index })</li>
				<%-- ${anything.index } 은 0부터 시작한다. --%>
			</c:forEach>
		</ul>
	</c:if>
	...
	<c:if test="${not empty requestScope.pointArr1}">
		<ul>
			<c:forEach var="point" items="${requestScope.pointArr1}" varStatus="anything">
				<li>${point}&nbsp;(카운트순서 &nbsp; ${anything.count })</li>
				<%-- ${anything.count } 은 1부터 시작한다. --%>
			</c:forEach>
		</ul>
	</c:if>
	...
	<h2>정수로 되어진 데이터를 세자리마다 콤마 찍어서 나타내기</h2>
	<c:set var="sum" value="0" />
